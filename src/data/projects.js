export const projects = [
    ...
    engineeringDecisions: [
      {
        ...
        rationale:
          'The domain is inherently relational — students, courses, faculty, attendance, fees, library. Relational integrity mattered more than schema flexibility.',
      },
      {
        decision: 'JWT over session-based auth',
        rationale:
          "Stateless tokens meant we didn't need a session store, simplifying horizontal scaling if the college ever adopted it.",
      },
      {
        decision: 'Monolithic repo with clear module boundaries',
        rationale:
          'A microservices split would have added deployment overhead for a 4-person student team. Module-level separation was enough.',
      },
    ],
    results:
  ...
  },
  {
    ...
    engineeringDecisions: [
      {
        decision: 'Whisper over cloud STT',
        rationale:
          "OpenAI Whisper runs locally and doesn't require internet for transcriptions — critical for hackathon demo reliability.",
      },
      {
        decision: 'gTTS over neural TTS',
        rationale:
          'gTTS is free, fast, and supports 100+ languages. Output quality was sufficient for the target use case (educational content).',
      },
    ],
    results:
  ...
  },
  {
    ...
    engineeringDecisions: [
      {
        decision: 'Cookie-based auth over localStorage JWT',
        rationale:
          'Cookies with HttpOnly flags are more resistant to XSS attacks, important for a two-sided platform handling payments.',
      },
      {
        decision: 'Socket.io for real-time updates',
        rationale:
          'WebSocket-based communication for ride status changes (requested → accepted → started → completed) eliminated the need for polling.',
      },
    ],
    results:
